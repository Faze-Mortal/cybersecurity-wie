...
(function init() {
    ...
    if (data.scores && data.scores.round_4 !== null && !isAdmin) {
        startScreen.classList.remove('active');
        if (blockedScreen) {
            blockedScreen.classList.add('active');
            const blockedScore = document.getElementById('blocked-score');
            if (blockedScore) blockedScore.textContent = data.scores.round_4;
        }
        return;
    }
...
async function endGame() {
    ...
    for (let i = 0; i < ROUND_4_ANSWERS.length; i++) {
        if (userAnswers[i] === ROUND_4_ANSWERS[i]) {
            serverScore++;
        }
    }
    ...
    if (!isAdmin) {
        ...
        sessionData.scores.round_4 = serverScore;
